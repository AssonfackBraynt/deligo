...
import type { ProviderProfilePrivate, ProviderType } from '@/features/provider-profile/profile-types';
...
export default function EditProviderProfilePage() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (isRider) {
        body.baseCity = values.baseCity?.trim() || undefined;
        body.serviceCoverage = values.serviceCoverage?.trim() || undefined;
      }
      if (isCompany) {
        body.businessAddress = values.businessAddress?.trim() || undefined;
        ...
        <Container className="max-w-2xl">
          <div className="mb-6 flex items-center gap-3">
            <Button asChild variant="ghost" size="sm">
              <Link href={routes.provider.myProfile}>
          ...
          <div className="mb-6">
            <h1 className="text-2xl font-semibold text-foreground">Edit provider profile</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Provider type:{' '}
              <span className="font-medium text-foreground">
                {providerType ? TYPE_LABELS[providerType] : '—'}
              </span>{' '}
              (cannot be changed)
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <Button asChild variant="ghost" size="lg">
                <Link href={routes.provider.myProfile}>Cancel</Link>
              </Button>
            </div>
          </form>
        </Container>
